package com.example.j2n.room_srv.filter;
...
public class InternalAuthFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (internalToken == null || !internalToken.equals(token)) {
            log.error("[ROOM-SRV] Invalid internal token: {}", token);
        ...
        if (userId == null || userId.isBlank()) {
            log.error("[ROOM-SRV] User ID is missing in headers");
            response.setStatus(HttpServletResponse.SC_UNAUTHORIZED);
            response.getWriter().write("Unauthorized: User info missing");
            return;
        }
