package com.example.j2n.product_srv.filter;
...
public class InternalAuthFilter extends OncePerRequestFilter {
    ...
    private static final List<String> NOT_FILTER_LIST = List.of("/product/graphiql", "/error");
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
        ...
        if (!internalToken.equals(token)) {
            log.error("[PRODUCT-SRV] Invalid internal token");
        ...
        if (userId == null || userId.isBlank()) {
            log.error("[PRODUCT-SRV] User not authenticated");
